 ...
 */
export default class ChartJs extends HTMLElement {
    ...
    connectedCallback() {
        ++ChartJs._iCount // increment total instance count

        // Create an id from name or calculation if needed
        this.name = this.getAttribute('name')
        if (!this.id) {
            if (this.name) this.id = this.name.toLowerCase().replace(/\s/g, '_')
            else this.id = `sc-${ChartJs._iCount}`
        }
 ...
 */
window[className] = ChartJs

// Self-register the HTML tag
customElements.define(componentName, ChartJs)
