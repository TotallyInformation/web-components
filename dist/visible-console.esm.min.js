var x=class C extends HTMLElement{static baseVersion="2025-09-19";static _iCount=0;uib=!!window.uibuilder;uibuilder=window.uibuilder;$;$$;connected=!1;name;opts={};static get version(){return`${this.componentVersion} (Base: ${this.baseVersion})`}constructor(){super()}config(e){return e&&(this.opts=C.deepAssign(this.opts,e)),this.opts}createShadowSelectors(){this.$=this.shadowRoot?.querySelector.bind(this.shadowRoot),this.$$=this.shadowRoot?.querySelectorAll.bind(this.shadowRoot)}static deepAssign(e,...n){for(let r of n)for(let i in r){let c=r[i],s=e[i];if(Object(c)==c&&Object(s)===s){e[i]=C.deepAssign(s,c);continue}e[i]=r[i]}return e}async doInheritStyles(){if(!this.shadowRoot||!this.hasAttribute("inherit-style"))return;let e=this.getAttribute("inherit-style");e||(e="./index.css");let n=document.createElement("link");n.setAttribute("type","text/css"),n.setAttribute("rel","stylesheet"),n.setAttribute("href",e),this.shadowRoot.appendChild(n),console.info(`[${this.localName}] Inherit-style requested. Loading: "${e}"`)}ensureId(){this.uib=!!window.uibuilder,this.id||(this.id=`${this.localName}-${++this.constructor._iCount}`)}hasSlotContent(){return this.shadowRoot.querySelector("slot").assignedNodes().some(r=>r.nodeType===Node.ELEMENT_NODE?!0:r.nodeType===Node.TEXT_NODE?r.textContent.trim().length>0:!1)}prependStylesheet(e,n=0){if(!e)throw new Error(`[${this.localName}] cssText must be provided`);let r=this._findExistingStylesheet();if(r)return r;let i=document.createElement("style");return i.textContent=e,i.setAttribute("data-component",this.localName),i.setAttribute("data-order",n.toString()),this._prependToDocumentHead(i,n),i}uibSend(e,n){this.uib&&(this.uibuilder.ioConnected?this.uibuilder.send({topic:`${this.localName}:${e}`,payload:n,id:this.id,name:this.name}):console.warn(`[${this.localName}] uibuilder not connected to server, cannot send:`,e,n))}_connect(){this.ensureId(),this.doInheritStyles(),this.uib&&this.uibuilder.onTopic(`${this.localName}::${this.id}`,this._uibMsgHandler.bind(this))}_construct(e,n){e&&(n||(n={mode:"open",delegatesFocus:!0}),this.attachShadow(n).append(e),this.createShadowSelectors())}_disconnect(){document.removeEventListener(`uibuilder:msg:_ui:update:${this.id}`,this._uibMsgHandler),this._event("disconnected")}_event(e,n){this.dispatchEvent(new CustomEvent(`${this.localName}:${e}`,{bubbles:!0,composed:!0,detail:{id:this.id,name:this.name,data:n}}))}_ready(){this.connected=!0,this._event("connected"),this._event("ready")}_uibMsgHandler(e){if(typeof e.payload!="object"){console.warn(`[${this.localName}] Ignoring msg, payload is not an object:`,e);return}Object.keys(e.payload).forEach(n=>{n.startsWith("_")||(this[n]=e.payload[n])})}_findExistingStylesheet(){return document.head.querySelector(`style[data-component="${this.localName}"]`)}_prependToDocumentHead(e,n){let r=document.head,i=Array.from(r.querySelectorAll("style[data-component]"));if(i.length===0){let s=r.firstChild;s?r.insertBefore(e,s):r.appendChild(e);return}let c=null;for(let s of i){let a=parseInt(s.getAttribute("data-order")??"0",10);if(n<a){c=s;break}}if(c)r.insertBefore(e,c);else{let a=i[i.length-1].nextSibling;a?r.insertBefore(e,a):r.appendChild(e)}}},E=x;var k=document.createElement("template");k.innerHTML=`
    <style>
        :host {
