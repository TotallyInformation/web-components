...
`;var a=class extends h{static componentVersion=c;elInput;elLabel;elErrorMessage;oldValues={};chgCount=0;isInForm=!1;static get observedAttributes(){return["inherit-style","name","type","value","placeholder","disabled","readonly","required","min","max","minlength","maxlength","size","autocomplete","autofocus","form","list","pattern","step","multiple","value"]}constructor(){super(),this.prependStylesheet(p,0);let e=Array.from(this.childNodes);this.innerHTML=`
            <label for="input"></label>
            <input type="text" />
            <error-message></error-message>
        `,this.elInput=this.querySelector("input"),this.elInput.id=`${this.id}-input`,this.elInput.setAttribute("name",`${this.id}-input`),this.elLabel=this.getElementsByTagName("LABEL")[0],this.elLabel.setAttribute("for",this.elInput.id),e.length>0&&(this.elLabel.innerHTML="",this.elLabel.append(...e)),this.elErrorMessage=this.querySelector("error-message")}connectedCallback(){this._connect();let e=this.closest("input-group");e&&(e.id||e.name)&&this.elInput.setAttribute("name",e.id||e.name);let t=this.closest("form");this.isInForm=!!t,this.uib=!!window.uibuilder,this.uib&&!this.isInForm&&this.elInput.addEventListener("change",this._valueChanged.bind(this),!0),this._ready()}disconnectedCallback(){this.uib&&!this.isInForm&&this.elInput.removeEventListener("change",this._valueChanged.bind(this),!0),this._disconnect()}attributeChangedCallback(e,t,i){if(t!==i){switch(this[e]=i,this.oldValues[e]=t,e){case"inherit-style":break;case"type":{this._updateType(i);break}default:{this.elInput?.setAttribute(e,i);break}}this._event("attribChanged",{attribute:e,newVal:i,oldVal:t})}}_updateType(e){this.elInput?.setAttribute("type",e||"text")}_valueChanged(e){if(++this.chgCount,setTimeout(()=>{this.chgCount=0},20),this.chgCount>1)return;let t;["checkbox","radio"].includes(this.elInput.type)?t=this.elInput.checked:t=isNaN(this.elInput.valueAsNumber)?this.elInput.value:this.elInput.valueAsNumber,window.uibuilder.send({topic:`labeled-input/change/${this.name||this.elInput.id}`,payload:t,_meta:{type:this.elInput.type,id:this.elInput.id,name:this.elInput.name,previous:this.oldValues.value||null}}),this.oldValues.value=this.elInput.value}};var u=class extends h{static componentVersion=c;static get observedAttributes(){return["inherit-style","name","type","title","frame-title","form-title"]}constructor(){super()}connectedCallback(){this._connect(),this._ready()}disconnectedCallback(){this._disconnect()}attributeChangedCallback(e,t,i){if(t!==i){switch(this[e]=i,e){case"type":{this._updateType(i);break}case"title":case"frame-title":case"form-title":{this._updateTitle(i);break}default:break}this._event("attribChanged",{attribute:e,newVal:i,oldVal:t})}}_updateType(e){if(!e)return;let t=Array.from(this.childNodes);["fieldset","frame"].includes(e)?(this.innerHTML=`
                <fieldset title="">
                    <legend></legend>
                </fieldset>
            ...
            `,this.elFrame=this.querySelector("form")),this.elFrame&&t.length>0&&this.elFrame.append(...t)}_updateTitle(e){if(!e||!this.elFrame)return;this.title=e;let t=this.elFrame.querySelector("div")||this.elFrame.querySelector("legend");t.innerText=e}},f=a;window.labeledInput=a;window.InputGroup=u;customElements.define("labeled-input",a);customElements.define("input-group",u);export{u as InputGroup,f as default,a as labeledInput};
/**
 * @class
 ...
 * See https://github.com/runem/web-component-analyzer?tab=readme-ov-file#-how-to-document-your-components-using-jsdoc
 */
//# sourceMappingURL=labeled-input.esm.min.js.map
